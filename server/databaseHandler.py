    ...
    try:
        ...
        try:
            DBcontent = cursor.execute(
                """SELECT content FROM notes WHERE board = ? AND level = ? AND subject = ? AND  topic = ? """,
                (board, level, subject, topic),
            ).fetchone()
        except Exception as e:
            return False
...
def getNote(subject: str, topic: str) -> dict:
    ...
    try:
        ...
        DBcontent = cursor.execute(
            """SELECT content FROM notes WHERE subject = ? AND topic = ? order by submitDate DESC""",
            (subject, topic),
        ).fetchone()
...
def rateNote(user_id: str, note_uuid: str, rating: int) -> bool:
    ...
    try:
        ...
        cursor.execute(
            """SELECT rating FROM note_ratings 
        ...
        if existing:
            # Update existing rating
            cursor.execute(
                """UPDATE note_ratings SET rating = ? 
                           WHERE user_id = ? AND note_uuid = ?""",
...
def searchNotes(
    subject: str = None, topic: str = None, min_rating: float = 0, min_views: int = 0
) -> list:
...
def convertRatingToString(rating: int) -> str:
    ...
    return {1: "one", 2: "two", 3: "three", 4: "four", 5: "five"}.get(rating, "unknown")
