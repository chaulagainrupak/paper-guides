            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((opt) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`w-4 h-4 flex-shrink-0 rounded border-2 flex items-center justify-center transition-colors ${checked ? "bg-[var(--blue-highlight)] border-[var(--blue-highlight)]" : "border-gray-400"}`}
                    >
                      {checked && (
  ...
  return (
    <div>
      <label className="block text-sm font-bold mb-2 uppercase tracking-wide opacity-60">
        📊 Level{" "}
        <span className="text-[10px] normal-case font-normal">(leave empty for all)</span>
      </label>
          ...
          levels.map((lvl) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold border-2 transition-all duration-200 hover:scale-105 select-none ${
                  checked
            ...
            <button
              ...
              className={`w-11 h-11 rounded-xl font-bold text-sm border-2 transition-all duration-200 hover:scale-110 select-none ${
                checked
                  ...
                  : "border-[var(--color-border)] text-[var(--font-color)] hover:border-opacity-60"
              }`}
...
export default function QuestionGeneratorForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    trackEvent("Question Generation Attempted", {
      ...
      "Topic Count": selectedTopics.length > 0 ? selectedTopics.length : availableTopics.length,
      Components: selectedComponents.length > 0 ? selectedComponents.join(", ") : "All Components",
      "Component Count": componentsToSend.length,
      Levels: selectedLevels.length > 0 ? selectedLevels.join(", ") : "All Levels",
      "Level Count": levelsToSend.length,
      Difficulties: selectedDifficulties.length > 0 ? selectedDifficulties.join(", ") : "All Difficulties",
      "Difficulty Count": difficultiesToSend.length,
      "Topics Filtered": selectedTopics.length > 0,
      "Components Filtered": selectedComponents.length > 0,
      "Levels Filtered": selectedLevels.length > 0,
      "Difficulties Filtered": selectedDifficulties.length > 0,
    });
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        trackEvent("Question Generation Auth Error", {
          Board: selectedBoard,
          Subject: selectedSubject,
        });
        ...
        trackEvent("Question Generation Failed", {
          ...
          "Error Preview": errText.slice(0, 100),
        });
  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl mx-auto rounded-2xl bg-[var(--baby-powder)] shadow-2xl">
      ...
      <div className="flex items-center justify-center gap-2 mb-8">
        {steps.map((step, i) => (
          <div key={step.label} className="flex items-center gap-2">
            <div
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold transition-colors ${step.done ? "bg-[var(--blue-highlight)] text-white" : "bg-[var(--color-nav)] opacity-60"}`}
            >
              {step.done && (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <LevelSection levels={availableLevels} selected={selectedLevels} setSelected={setSelectedLevels} />
          <DifficultySection selected={selectedDifficulties} setSelected={setSelectedDifficulties} />
        </div>
