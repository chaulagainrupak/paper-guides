document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleAllSelection(containerId, inputName) {
    ...
    const otherCheckboxes = container.querySelectorAll(
      `input[name="${inputName}"]:not([value="ALL"])`,
    );
  ...
  function updateComponents(system) {
    ...
    config[system].components.forEach((component) => {
      ...
      checkbox.addEventListener("change", function () {
        const allCheckbox =
          componentCheckboxes.querySelector('input[value="ALL"]');
        if (this.checked) {
  ...
  function populateTopics(subjectName) {
    ...
    const subject = config[currentSystem].subjects.find(
    ...
    if (subject) {
      ...
      subject.topics.forEach((topic) => {
        ...
        checkbox.addEventListener("change", function () {
          const allCheckbox =
            topicCheckboxes.querySelector('input[value="ALL"]');
          if (this.checked) {
            allCheckbox.checked = false;
            handleAllSelection("topicCheckboxes", "topic");
          }
  ...
  function updateSelectTitle(checkboxesId, titleClass) {
    ...
    const checkedBoxes = checkboxes.querySelectorAll(
      'input[type="checkbox"]:checked',
    );

    if (checkedBoxes.length === 0) {
      titleElement.textContent =
        titleClass === "topics-title"
          ? "Select Topic(s)"
          : titleClass === "difficulty-title"
            ? "Select Difficulty"
            : "Select Component(s)";
    } else {
  ...
  const difficultyBoxes = difficultyCheckboxes.querySelectorAll(
    'input[type="checkbox"]:not([value="ALL"])',
  );
  difficultyBoxes.forEach((checkbox) => {
    checkbox.addEventListener("change", function () {
      const allCheckbox =
        difficultyCheckboxes.querySelector('input[value="ALL"]');
      if (this.checked) {
        allCheckbox.checked = false;
        handleAllSelection("difficultyCheckboxes", "difficulty");
      }
  ...
  window.toggleCheckboxes = function (checkboxesId) {
    ...
    const isCurrentlyHidden =
      checkboxes.style.display === "none" || checkboxes.style.display === "";
